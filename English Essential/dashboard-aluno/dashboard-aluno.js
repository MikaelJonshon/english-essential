...
async function openLesson(lessonId, lessonTitle, pathClassIndex = null) {
  ...
  if (allAnswered && currentLesson.exercises.length > 0) {
    currentLesson.exercises.forEach(e => { if (e.answered) answers[e.id] = { is_correct: e.answered.is_correct, xp_earned: 0 }; });
...
function renderExercise() {
  ...
  document.getElementById('exProgressLabel').textContent = `Exercício ${idx + 1} de ${total}`;
  ...
  const typeLabels = {
    ...
    is_or_are:'IS ou ARE?', image_choice:'Toque na imagem certa', word_order:'Monte a frase'
  };
  const typeIcons  = {
    ...
    image_choice:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>',
  ...
  if (ex.question_type === 'speaking' && ex.audio_url) setTimeout(() => playAudio(ex.audio_url), 300);
}
...
async function confirmAnswer() {
  ...
  else { answer = (document.getElementById('textAnswer')?.value || '').trim(); if (!answer) return; }
...
function nextExercise() { stopMic(); if (_currentAudio) { _currentAudio.pause(); _currentAudio = null; } currentExIdx++; renderExercise(); }
...
function cfgApplyToUI() {
  ...
  document.getElementById('cfgToggleDarkBlue').checked   = cfgData.darkBlue;
...
function cfgSave() {
  ...
  cfgData.darkBlue        = document.getElementById('cfgToggleDarkBlue').checked;
